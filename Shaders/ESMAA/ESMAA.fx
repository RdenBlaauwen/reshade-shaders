...
uniform bool TSMAASofteningTest <
	ui_label = "Use TSMAA softening";
	ui_category = "Image Softening";
> = false;
...
float3 HQAASofteningPS(float4 vpos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    float4 edgedata = HQAA_Tex2D(blendSampler, texcoord);
	...
	if (possiblediag)
	{
		bool4 nearbydiags = bool4(
			all(HQAA_Tex2D(blendSampler, texcoord + float2(BUFFER_RCP_WIDTH, BUFFER_RCP_HEIGHT))), 
			all(HQAA_Tex2D(blendSampler, texcoord - float2(BUFFER_RCP_WIDTH, BUFFER_RCP_HEIGHT))), 
			all(HQAA_Tex2D(blendSampler, texcoord + float2(-BUFFER_RCP_WIDTH, BUFFER_RCP_HEIGHT))), 
			all(HQAA_Tex2D(blendSampler, texcoord + float2(BUFFER_RCP_WIDTH, -BUFFER_RCP_HEIGHT)))
		);
...
//  i c d
	...
	{
    	float spuriousthreshold = 
			rcp(__HQAA_EDGE_THRESHOLD * rcp(edgedata.a)) 
			* saturate(HqaaSoftenerSpuriousThreshold);
